	...
	public class ContentPageHelper : IDisposable {
		...
		public PageViewType GetBlogHeadingFromURL(SiteData site, string blogUrl) {
			...
			if (site.CheckIsBlogCategoryPath(blogUrl)) {
				...
				var query = CompiledQueries.cqGetCategoryByURL(_db, siteID, blogUrl);
				if (query != null) {
			...
			if (site.CheckIsBlogTagPath(blogUrl)) {
				...
				var query = CompiledQueries.cqGetTagByURL(_db, siteID, blogUrl);
				if (query != null) {
			...
			if (site.CheckIsBlogEditorFolderPath(blogUrl)) {
				...
				var query = CompiledQueries.cqGetEditorByURL(_db, siteID, blogUrl);
				if (query != null) {
			...
			if (site.CheckIsSiteSearchPath(blogUrl)) {
				...
				if (CarrotHttpHelper.QueryString(SiteData.SearchQueryParameter) != null) {
					searchTerm = CarrotHttpHelper.QueryString(SiteData.SearchQueryParameter).ToString();
				}
