...
namespace Carrotware.CMS.Core {
	...
	public partial class SiteData : IValidatableObject {
		...
		public SiteData(CarrotSite s) {
			...
			this.Blog_Root_ContentID = s.BlogRootContentId;

			this.Blog_FolderPath = string.IsNullOrEmpty(s.BlogFolderPath) ? "" : s.BlogFolderPath;
			this.Blog_CategoryPath = string.IsNullOrEmpty(s.BlogCategoryPath) ? "" : s.BlogCategoryPath;
			this.Blog_TagPath = string.IsNullOrEmpty(s.BlogTagPath) ? "" : s.BlogTagPath;
			this.Blog_EditorPath = string.IsNullOrEmpty(s.BlogTagPath) ? "" : s.BlogEditorPath;
			this.Blog_DatePath = string.IsNullOrEmpty(s.BlogDatePath) ? "" : s.BlogDatePath;
			this.Blog_DatePattern = string.IsNullOrEmpty(s.BlogDatePattern) ? "yyyy/MM/dd" : s.BlogDatePattern;
		...
		public void Save() {
			using (CarrotCakeContext _db = CarrotCakeContext.Create()) {
				...
				s.BlogFolderPath = ContentPageHelper.ScrubSlug(this.Blog_FolderPath);
				s.BlogCategoryPath = ContentPageHelper.ScrubSlug(this.Blog_CategoryPath);
				s.BlogTagPath = ContentPageHelper.ScrubSlug(this.Blog_TagPath);
				s.BlogEditorPath = ContentPageHelper.ScrubSlug(this.Blog_EditorPath);
				s.BlogDatePath = ContentPageHelper.ScrubSlug(this.Blog_DatePath);

				s.BlogRootContentId = this.Blog_Root_ContentID;
		...
		public void RenderRSSFeed(HttpContext context) {
			...
			if (!string.IsNullOrEmpty(CarrotHttpHelper.QueryString("type"))) {
				string feedType = CarrotHttpHelper.QueryString("type");
