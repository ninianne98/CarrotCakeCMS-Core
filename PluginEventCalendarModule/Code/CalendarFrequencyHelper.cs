...
namespace CarrotCake.CMS.Plugins.EventCalendarModule {
	...
	public class CalendarFrequencyHelper {
		...
		public static void SaveFrequencies(CalendarContext ctx, CalendarEvent newItem, CalendarEvent oldItem) {
			...
			if (oldItem.SiteID == Guid.Empty && oldItem.CalendarFrequencyID == Guid.Empty) {
				...
				int iMaxRange = (from c in ctx.CalendarEvents
				...
				if (oldItem.Frequency != newItem.Frequency || oldItem.RecursEvery != newItem.RecursEvery
					...
					var lstEvents = (from c in ctx.CalendarEvents
									 orderby c.EventDate
									 where c.CalendarEventProfileId == newItem.CalendarEventProfileID
									 select c).ToList();
					...
					var delIds = lstDel.Select(x => x.CalendarEventId).ToList();
					ctx.CalendarEvents.Where(x => delIds.Contains(x.CalendarEventId)).ExecuteDelete();
		...
		protected static void InsertEventsFromList(CalendarContext ctx, CalendarEvent item, List<DateTime> lstDates) {
			SiteData site = SiteData.CurrentSite;

			foreach (DateTime date in lstDates) {
				var evt = new CalendarSingleEvent {
		...
		public static List<CalendarFrequency> GetCalendarFrequencies() {
			using (var db = CalendarContext.GetDataContext()) {
				return (from c in db.CalendarFrequencies
						orderby c.FrequencySortOrder
						select c).ToList();
			}
		}
