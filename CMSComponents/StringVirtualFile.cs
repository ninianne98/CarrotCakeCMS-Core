...
namespace Carrotware.CMS.UI.Components {
	...
	public class VirtualViewFileProvider : IFileProvider {
		private readonly IServiceProvider _serviceProvider;

		internal static string VirtualPath = "~/Virtual/CmsInMemory/";
		...
		public VirtualViewFileProvider(IServiceProvider serviceProvider) {
		...
		internal VirtualView GetContent(string path) {
			var result = new VirtualView();

			if (PathProbably(path)) {
		...
		public static string RegisterView(string scriptKey) {
			...
			if (_virtualFiles.ContainsKey(key) == false) {
				var viewText = CarrotCakeHtmlWeb.GetEmbededView(scriptKey);
				var namespaces = CarrotCakeHtmlWeb.GetRazorNamespaces();
				...
				var nsUsings = string.Join(Environment.NewLine, namespaces);
		...
		public IFileInfo GetFileInfo(string path) {
			var viewPath = ScrubFilename(path);

			if (PathProbably(viewPath) == false) { return new NotFoundFileInfo(path); }

			var view = GetContent(viewPath);

			if (string.IsNullOrWhiteSpace(view.Markup) || view.LoadDate == DateTime.MinValue) {
				return new NotFoundFileInfo(viewPath);
			}

			return new VirtualViewFileInfo(view);
		}
	...
	public class VirtualViewFileInfo : IFileInfo {
		private readonly VirtualView _view;

		public VirtualViewFileInfo(VirtualView view) => _view = view;

		public bool Exists => true;
		public long Length => Encoding.UTF8.GetByteCount(_view.Markup);
		public string? PhysicalPath => null; // Indicates it doesn't live on disk
		public string Name => Path.GetFileName(_view.Path);
		public DateTimeOffset LastModified => _view.LoadDate;
		public bool IsDirectory => false;

		public Stream CreateReadStream() {
			return new MemoryStream(Encoding.UTF8.GetBytes(_view.Markup));
		}
	}
}