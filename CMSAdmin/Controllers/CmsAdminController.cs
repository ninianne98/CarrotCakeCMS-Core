	...
	public class CmsAdminController : Controller {
		...
		public override void OnActionExecuting(ActionExecutingContext context) {
			...
			if (this.HttpContext.User.Identity.IsAuthenticated) {
				...
				try {
					...
					SiteData.WriteDebugException("onactionexecuting", ex);
					throw;
				}
			}
		}
		...
		public async Task<ActionResult> UserProfile(ExtendedUserData model) {
			if (ModelState.IsValid) {
				IdentityResult result = await securityHelper.UserManager.SetEmailAsync(SecurityData.CurrentIdentityUser, model.Email);
		...
		public JsonResult AjaxFileUpload(AjaxFileUploadModel model) {
			List<IFormFile> files = model.PostedFiles.ToList();
		...
		[AllowAnonymous]
		[ValidateAntiForgeryToken]
		public async Task<ActionResult> CreateFirstAdmin(RegisterViewModel model) {
			RedirectIfUsersExist();
		...
		public async Task<ActionResult> Login(LoginViewModel model) {
			...
			if (result && user != null && exUser != null & exUser.IsLocked == false) {
				...
				if (!SecurityData.IsAuthEditor) {
					return RedirectToAction(this.GetActionName(x => x.NotAuthorized()));
				}
		...
		public ActionResult RoleAddEdit(RoleModel model) {
			...
			if (ModelState.IsValid) {
				...
				var role = SecurityData.FindRoleByID(model.Role.RoleId);
				if (role == null) {
					role = SecurityData.FindRole(model.Role.RoleName);
				}

				if (role != null) {
					item = SecurityData.GetUserRole(role.Id);
				}
				...
				item.RoleName = model.Role.RoleName.Trim();
